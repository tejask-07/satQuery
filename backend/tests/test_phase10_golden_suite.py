...
# ============================================================
# HELPER: SAFE ACCESS TO QueryPlan
# ============================================================

def get_plan_value(plan, key, default=None):
    """
    Safely read a field from QueryPlan whether it is:
...
def test_golden_semantic_recovery_multi_temporal_mode():
    ...
    # FIX: QueryPlan is not necessarily a dict
    temporal_mode = get_plan_value(res.plan, "temporal_mode")

    assert temporal_mode == "persistence_reversal"
    ...
    veg_data = (
        res.temporal_analysis.get("domains", {}).get("vegetation", {})
        or res.temporal_analysis.get("vegetation", {})
    )
...
def test_golden_semantic_bitemporal_persistence_suppressed():
    """For N=2 bi-temporal queries, domain persistence must be suppressed."""
    queries = {q.id: q for q in load_golden_manifest()}
    ...
    assert temporal_mode == "bi_temporal"
...
def test_golden_semantic_calibration_vocabulary():
    ...
    assert cal["observation_reliability"]["state"] in {
        "high",
        "moderate",
        "low",
        "unavailable",
    ...
    assert cal["semantic_evidence"]["state"] in {
        "very_strong",
        "strong",
        "moderate",
        "weak",
    ...
    assert cal["spatial_assessment"]["state"] in {
        "high",
        "moderate",
        "low",
        "unavailable",
    ...
    assert cal["temporal_consistency"]["state"] in {
        "high",
        "moderate",
        "limited",
        "bi_temporal_only",
